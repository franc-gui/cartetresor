...
import com.carbonit.cartetresor.domain.cartetresor.entity.Orientation;
...
public class ProcessCarteTresorUseCase implements IProcessCarteTresor {
        ...
        fileLines.forEach(line -> {
            if (!line.startsWith("#")) {
                ...
                switch (parts[0]) {
                    ...
                    case "A" ->
                            aventuriers.add(new Aventurier(parts[1], new Position(Integer.parseInt(parts[2]), Integer.parseInt(parts[3])), Orientation.valueOf(parts[4]), parts[5], 0));
                    default ->
    ...
    private List<String> convertToFilelines(Carte carte, List<Aventurier> aventuriers) {
        ...
        aventuriers.forEach(aventurier -> filelines.add("A - " + aventurier.getNom() + " - " + aventurier.getPosition().getX() + " - " +
                aventurier.getPosition().getY() + " - " + aventurier.getOrientation() + " - " + aventurier.getNbTresorsRamasses()));
        return filelines;
    }
